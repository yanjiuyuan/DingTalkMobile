...
Page({
    ...
    editor(e) {
        ...
        if (firstNeedArray.length > 0) {
            ...
            if (nodeInfo.RoleNames) {
                let RoleNames = nodeInfo.RoleNames.split(',');
                for (let i = 0, len = RoleNames.length; i < len; i++) {
                    let index = this.data.roleList.findIndex(item => {
                        return item == RoleNames[i];
                    })
                    choosePeopleArray[i].roleIndex = index;
    ...
    changeArray() {
        ...
        if (this.data.ChoseNodeId) {
            for (let i = 0, len = this.data.ChoseNodeId.length; i < len; i++) {
                if (this.data.firstNeedArray.length > 0 && this.data.firstNeedArray[i].checked) {
                    IsSelectMore[i] = "1";
                } else {
                    IsSelectMore[i] = "0";
                }
            ...
            for (let i = 0, len = this.data.ChoseNodeId.length; i < len; i++) {
                if (this.data.secondNeedArray.length > 0 && this.data.secondNeedArray[i].checked) {
                    IsMandatory[i] = "1";
                } else {
                    IsMandatory[i] = "0";
                }
            ...
            for (let i = 0, len = this.data.ChoseNodeId.length; i < len; i++) {
                if (this.data.thirdNeedArray.length > 0 && this.data.thirdNeedArray[i].checked) {
                    ChoseType[i] = "1";
                } else {
                    ChoseType[i] = "0";
                }
