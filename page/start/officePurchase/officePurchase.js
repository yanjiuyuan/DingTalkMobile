...
Page({
	...
	search(e) {
		...
		this.getDataReturnData('OfficeSuppliesPurchase/GetTable' + this.formatQueryStr({ startTime: value.startTime, endTime: value.endTime + " 23:59:59" }), (res) => {
			...
			for (let d of res.data) {
				for (let l of this.data.dataList) {
					if (d.dept == l.name) {
						this.data.totalPrice = (parseFloat(this.data.totalPrice) + parseFloat(d.ExpectPrice != "" ? d.ExpectPrice : 0) * parseFloat(d.Count)).toFixed(2);
						d['totalPrice'] = (parseFloat(d.ExpectPrice != "" ? d.ExpectPrice : 0) * parseFloat(d.Count)).toFixed(2);
						l.value.push(d);
						l.tableParam.total++;
		...
		if (!e.target.targetDataset.opt2) {
			...
			for (let i of this.data.dataList) {
				for (let j of i.value) {
					if (row.Id == j.Id) {
						i.tmpTotalPrice = (parseFloat(i.tmpTotalPrice) - parseFloat(row.ExpectPrice != "" ? row.ExpectPrice : 0) * parseFloat(row.Count)).toFixed(2);
						this.data.totalPrice = (parseFloat(this.data.totalPrice) - parseFloat(row.ExpectPrice != "" ? row.ExpectPrice : 0) * parseFloat(row.Count)).toFixed(2);
	...
	bindAll(taskId) {
		...
		that._postData("OfficeSuppliesPurchase/SaveTable", function(res) {
			that.doneSubmit();
		}, paramArr)
	}
})